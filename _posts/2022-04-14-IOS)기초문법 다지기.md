...
### 즉 a부터b-1까지 값을 갖는다. 보통 배열이 배열의 크기 -1의 인덱스를 갖기 때문에 이 반닫힌 연산자는 배열을 다루는데 유용하다. 편하게 0부터 인덱스가 시작된다고 보면된다. (0,1,2,3)이런식으로..
```swift
...
```
### 단방향 범위 연산자는 subscript뿐만 아니라 아래와같이 특정 값을 포합하는지 여부를 확일할 때도 사용한다.
~~~swift
let range = ...5
...
// Prints "ACCESS DENIED -->> enteredDoorCode 그리고 passedRetinaScan이다. 즉, true그리고 false라서 else 문으로 프린트된거다."
```
...
// Prints "Welcome! -->> hasDoorKey 또는 knowsOverridePassword이다. 즉, false 또는 true여서 참으로 프린트 된거다."
```
# `논리 연산자의 조합`
...
/* Prints "Welcome! ->> enteredDoorCode(true) 그리고 passedRetinaScan(false) 또한 hasDoorKey(false) 또한 knowsOverridePassword(true)이다.
 ...
 enteredDoorCode(true) 그리고 passedRetinaScan(false)으로 else문으로 넘어갈뻔했지만
 또한 hasDoorKey(false) 또한 knowsOverridePassword(true)이부분에서 트루로 프린트된거다.
 */
